...
class ConstantLengthDataset(IterableDataset):
    ...
    def __iter__(self):
        ...
        while more_examples:
            buffer_prefix, buffer_len_prefix = [], 0
            ...
            while True:
                ...
                try:
                    ...
                    buffer_prefix.append(next_entry["prefix"])
                    buffer_middle.append(next_entry["middle"])
                    buffer_suffix.append(next_entry["suffix"])
                    buffer_len_prefix += len(buffer_prefix[-1])
                    buffer_len_middle += len(buffer_middle[-1])
                    buffer_len_suffix += len(buffer_suffix[-1])
                    ...
                    if self.infinite:
                        ...
                        more_examples = False
                        break
            tokenized_inputs_prefix = self.tokenizer(buffer_prefix, truncation=False,
                                                     add_special_tokens=False)["input_ids"]
            tokenized_inputs_middle = self.tokenizer(buffer_middle, truncation=False,
                                                     add_special_tokens=False)["input_ids"]
            tokenized_inputs_suffix = self.tokenizer(buffer_suffix, truncation=False,
                                                     add_special_tokens=False)["input_ids"]
            all_token_ids = []

            for tokenized_input_prefix, tokenized_input_middle, tokenized_input_suffix in (
                    zip(tokenized_inputs_prefix, tokenized_inputs_middle, tokenized_inputs_suffix)
            ):
                tokenized_input = tokenized_input_prefix + tokenized_input_middle + tokenized_input_suffix
                # optionally do FIM permutations
                if self.fim_rate > 0:
                    tokenized_input, np_rng = fim.permute(
                        tokenized_input,
                        tokenized_input_prefix,
                        tokenized_input_middle,
                        tokenized_input_suffix,
                        np_rng,
                        self.suffix_tok_id,
                        self.prefix_tok_id,
                        self.middle_tok_id,
...
def create_datasets(tokenizer, args, seed):
    dataset = load_dataset("ASSERT-KTH/repairllama-datasets", "ir4xor2",
                           data_files={"train": "data/ir4xor2/train.jsonl", "eval": "data/ir4xor2/test.jsonl"})
    dataset = dataset.map(divide_affixes).select_columns(["prefix", "middle", "suffix"])
